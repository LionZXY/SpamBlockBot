...
val NICKNAME_REGEXP =
    "(^|\\s)/[a-zA-Z@\\d_]{1,255}|(^|\\s|\\()@[a-zA-Z\\d_]{1,32}|(^|\\s|\\()#[\\w.]+|(^|\\s)\\$[A-Z]{3,8}([ ,.]|$)".toRegex() //From https://github.com/DrKLO/Telegram/blob/e397bd9afdfd9315bf099f78a903f8754d297d7a/TMessagesProj/src/main/java/org/telegram/messenger/MessageObject.java#L2837
val DENIED_NICKNAME = listOf(
    "hoimsufa235", "kladov_oper", "klds_bot24", "restorantumen",
    ...
    "BestModerator", "qymyz07", "Oraku1", "dayhydra", "Hydrar4ever", "hydraday", "insideman_insider_bot"
).map { it.toLowerCase() }.toHashSet()

val DENIED_URL = listOf(
    "bit.ly", "t.me", "t.cn", "tinyurl.com", "autofleet.bz", "xinvst.com", "ton-gram.info",
...
object SpamBlocker {
    ...
    private fun onFindSpam(msg: Message, bot: SpamBlockBot) {
        val notifyDeleteMessage = SendMessage.builder()
            .chatId(Credentials.get(CredentialsEnum.USERID_LOG))
            .text("Удалено сообщение: \n\n ```${msg}```")
            .build()
        notifyDeleteMessage.enableMarkdown(true)
        ...
        try {
            messageToDeleted.forEach {
                deleteMessage(msg.chatId.toString(), it, bot)
            }

            bot.execute(notifyDeleteMessage)
        } catch (e: Exception) {
            e.printStackTrace()
            val notifyErrorMessage = SendMessage.builder()
                .chatId(Credentials.get(CredentialsEnum.USERID_ERRORREPORT))
                .text("Ошибка удаления сообщения (${e.message}) в чате c id ${msg.chatId}: $msg")
                .build()
            bot.execute(notifyErrorMessage)
        }
    }

    private fun deleteMessage(chatId: String, messageId: Int, bot: SpamBlockBot) {
        val deleteMessageMethod = DeleteMessage.builder()
            .chatId(chatId)
            .messageId(messageId)
            .build()
        ...
        try {
            ...
            e.printStackTrace()
            val notifyErrorMessage = SendMessage.builder().chatId(Credentials.get(CredentialsEnum.USERID_ERRORREPORT))
                .text("Ошибка удаления сообщения: ${e.localizedMessage} в чате c id $chatId: $messageId")
                .build()
            bot.execute(notifyErrorMessage)
        }
    }
